...
function makeFood(){
    if(food == 0 && dyn == 0 && sci ==0){
        foodId = 'col' + Math.floor(Math.random()*(N)) + Math.floor(Math.random()*(N));
...
function scissors(){
    if(score % 50 == 0){
        sciId = 'col' + Math.floor(Math.random()*(N)) + Math.floor(Math.random()*(N));
...
function obstacles(){
    if(score % 10 == 0){
        var obstId = 'col' + Math.floor(Math.random()*(N)) + Math.floor(Math.random()*(N));
        while(SNK.indexOf(obstId) !== -1 || OBS.indexOf(obstId) !== -1){
            obstId = 'col' + Math.floor(Math.random()*(N)) + Math.floor(Math.random()*(N));
        }
...
function dynamit(){
    if(score % 50 == 0){
        dynId = 'col' + Math.floor(Math.random()*(N)) + Math.floor(Math.random()*(N));
...
function Snake(event){
    ...
    for(let i = 0; i < SNK.length; i++){
        if(event.target.id == SNK[i] && event.target.style.backgroundImage == 'url("./image/tail3.png")'){
            console.log("Atsitrenkei į uodegą");
            endGame();
        }
    }
    //saugo gyvatės id
    ...
    for(let i = 0; i < OBS.length; i++){
        if(SNK.indexOf(OBS[i]) !== -1){
            console.log("Atsitrenkei į akmenį");
            endGame();
        }
    }
    //zirkles
